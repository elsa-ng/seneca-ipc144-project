...
void removeRecord(struct Item *item, int *NoOfRecs, int searchIndex);
void nameSearch(const struct Item item[], int NoOfRecs);
	...
	if (loadStatus == 0) {
		...
		do {
			...
			case 5:
				...
				break;
			case 6:
				deleteItem(items, &numRecords);
				saveCheck(items, DATAFILE, numRecords);
				pause();
				break;
			case 7:
				nameSearch(items, numRecords);
				break;
			case 0:
...
// Prompts user for SKU input and locates the item using the SKU input by user
// If found and user confirms the deletion, remoeRecord() is called to remove the item
void deleteItem(struct Item item[], int* NoOfRecs) {
	int skuNO, searchIndex = 0, found, answer;
	...
	if (found == 1) {
		...
		if (answer == 1) {
			removeRecord(item, NoOfRecs, searchIndex);
			printf("Deleted!\n");
		}
		else {
			printf("--== Aborted! ==--\n");
...
// Replaces item at searchIndex position with item of the next
// Reduces the NoOfRecs by 1 if conditions are met
// The last two records of the item struct will be duplicated,
// but once item is saved to a file, the very last record will be discarded
void removeRecord(struct Item *item, int *NoOfRecs, int searchIndex) {

	int i, last_index;

	// Checks to make sure searchIndex and *NoOfRecs are within bounds
	if (searchIndex < *NoOfRecs && searchIndex > -1 && *NoOfRecs > 0) {
		last_index = *NoOfRecs - 1;

		for (i = searchIndex; i < last_index; i++) {
			item[i] = item[i + 1];
		}
...
// Searches item by name and is case sensitive
// Displays each item that a match is found
void nameSearch(const struct Item item[], int NoOfRecs) {

	int i, matchCounter = 0;
	char searchString[MAX_STRING_LEN];

	printf("Please enter part of the name: ");
	scanf(" %[^\n]%*c", searchString);

	for (i = 0; i < NoOfRecs; i++) {
		if (strstr(item[i].name, searchString) != NULL) {
